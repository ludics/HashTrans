...
CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.launch --nproc_per_node 2 --master_port 13395 main_hash_part.py \
    --cfg configs/swin_part_base_patch4_window7_224.yaml  --data-path /data/ludi/datasets/Dogs \
    --batch-size 64 --accumulation-steps 2 --hash_bit 64 --tag dogs_hash_64_swin_part_tiny_clsm1 \
    --resume downloads/swin_base_patch4_window7_224_22k.pth --pretrained downloads/swin_base_patch4_window7_224_22k.pth \
    --valid_rank 1 --amp-opt-level O0 --dataset Dogs
