...
CUDA_VISIBLE_DEVICES=6,7 python -m torch.distributed.launch --nproc_per_node 2 --master_port 12370 main_hash.py \
    --cfg configs/swin_tiny_patch4_window7_224.yaml  --data-path /data/ludi/datasets/Dogs \
    --batch-size 64 --accumulation-steps 2 --hash_bit 64 --tag dogs_hash_64_swin_tiny_clsm1 \
    --resume downloads/swin_tiny_patch4_window7_224.pth --pretrained downloads/swin_tiny_patch4_window7_224.pth \
