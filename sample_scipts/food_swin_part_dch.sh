...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node 2 --master_port 13405 main_hash_part.py \
    --cfg configs/swin_part_tiny_patch4_window7_224_food.yaml  --data-path /data1/ludi/datasets/food-101 \
    --batch-size 62 --accumulation-steps 2 --hash_bit 16 --tag food101_hash_16_swin_part_tiny_clsm1 \
    --resume downloads/swin_tiny_patch4_window7_224.pth --pretrained downloads/swin_tiny_patch4_window7_224.pth \
    --valid_rank 0 --amp-opt-level O0 --dataset Food101
