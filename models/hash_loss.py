...
class DCHLoss(nn.Module):
    def __init__(self, config):
        ...
        self.gamma = config.HASH.GAMMA
        self.lambd = config.HASH.LAMBD
        self.K = config.HASH.HASH_BIT
        self.one = torch.ones((config.DATA.BATCH_SIZE, self.K)).cuda()
    ...
    def forward(self, u, y):
        ...
        return loss, cauchy_loss.mean(), quantization_loss.mean()
