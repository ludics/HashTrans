...
class SwinTransformer_Pyramid(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, pretrained_path=None, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer_P(dim=int(embed_dim * 2 ** i_layer),
            ...
            if (i_layer < self.num_layers - 1):
                pyramid = PatchMerging(input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                    patches_resolution[1] // (2 ** i_layer)),
    ...
    def init_pyramids(self, ckpt):
        ckpt_pyramids = collections.OrderedDict()
        for k, v in ckpt['model'].items():
            if 'downsample' in k:
                k_1 = k.replace('layers.', '').replace('downsample.', '')
                ckpt_pyramids[k_1] = v
                #del ckpt['model'][k]
        self.pyramids.load_state_dict(ckpt_pyramids, strict=False)
    ...
    def forward_features(self, x):
        ...
        #     x = layer(x)
        x, x_b_0 = self.layers[0](x)
        x_b_0 = self.pyramids[0](x_b_0)
        x, x_b_1 = self.layers[1](x)
        x_b_1 = self.pyramids[1](x_b_1 + x_b_0)
        # x_b_1 = self.pyramids[1](x_b_1)
        x, x_b_2 = self.layers[2](x)
        x_b_2 = self.pyramids[2](x_b_2 + x_b_1)
        x, _ = self.layers[3](x)
        x = x + x_b_2
...
class GlobalDescriptor(nn.Module):
    ...
    def forward(self, x):
        assert x.dim() == 3, 'the input tensor of GlobalDescriptor must be the shape of [B, L, C]'
        if self.p == 1:
            return torch.flatten(F.adaptive_avg_pool1d(x, output_size=1), 1)
        elif self.p == float('inf'):
            return torch.flatten(F.adaptive_max_pool1d(x, output_size=1), 1)
        else:
...
class SwinTransformer_Pyramid_M2(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, method=None, pretrained_path=None, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
        ...
        num_features_1 = embed_dim
        num_features_2 = 2 * embed_dim
        num_features_3 = 4 * embed_dim
        num_features_4 = 8 * embed_dim
        ...
        self.norm = norm_layer(num_features_4)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.maxpool = nn.AdaptiveMaxPool1d(1)
        # self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        self.toplayer = nn.Linear(num_features_4, 2 * embed_dim, bias=False)
    ...
    def _upsample_add(self, x, y, i_layer=0):
        B, Ly, C = y.shape
    ...
    def forward_features(self, x):
        ...
        #     x = layer(x)
        # Bottom-up
        c1, d1 = self.layers[0](x)
        c2, d2 = self.layers[1](c1)
        c3, d3 = self.layers[2](c2)
        c4, d4 = self.layers[3](c3)
...
class SwinTransformer_Pyramid_M4(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, method=None, pretrained_path=None, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.smooth2 = nn.Linear(self.pyramid_features, 1, bias=False)
        self.smooth1 = nn.Linear(self.pyramid_features, 1, bias=False)
    ...
    def forward_features(self, x):
        ...
        p4 = self.norm_pyramid4(p4)
        m4 = F.softmax(self.smooth4(p4), dim=1).repeat(1, 1, self.pyramid_features)
        p4 = m4 * p4
        p4 = torch.sum(p4, dim=1)
        p3 = self.norm_pyramid3(p3)
        m3 = F.softmax(self.smooth3(p3), dim=1).repeat(1, 1, self.pyramid_features)
        p3 = m3 * p3
        p3 = torch.sum(p3, dim=1)
        p2 = self.norm_pyramid2(p2)
        m2 = F.softmax(self.smooth2(p2), dim=1).repeat(1, 1, self.pyramid_features)
        p2 = m2 * p2
        p2 = torch.sum(p2, dim=1)
        p1 = self.norm_pyramid1(p1)
        m1 = F.softmax(self.smooth1(p1), dim=1).repeat(1, 1, self.pyramid_features)
        p1 = m1 * p1
...
class SwinTransformer_Pyramid_M3(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
...
class SwinTransformer_Pyramid_M5(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, method=None, pretrained_path=None, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.layer2_pool = GlobalDescriptor(3.0)
        self.layer1_pool = GlobalDescriptor(4.0)
...
class SwinTransformer_Pyramid_M6(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, method=None, pretrained_path=None, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.norm_pyramid2 = norm_layer(self.layer_features_2)
        self.norm_pyramid3 = norm_layer(self.layer_features_3)
        self.norm_pyramid4 = norm_layer(self.layer_features_4)

        self.uplayer1 = nn.Linear(self.layer_features_1, self.layer_features_2, bias=False)
        self.uplayer2 = nn.Linear(self.layer_features_2, self.layer_features_3, bias=False)
        self.uplayer3 = nn.Linear(self.layer_features_3, self.layer_features_4, bias=False)
    ...
    def forward_features(self, x):
        ...
        p4 = self.norm_pyramid4(d4)
        m4 = F.softmax(torch.sum(p4, axis=2), dim=1).unsqueeze(axis=2).repeat(1, 1, self.layer_features_4)
        p4 = m4 * p4
        p4 = torch.flatten(self.avgpool(p4.transpose(1, 2)), 1)
        # p4 = torch.sum(p4, dim=1)
        p3 = self.norm_pyramid3(d3)
        m3 = F.softmax(torch.sum(p3, axis=2), dim=1).unsqueeze(axis=2).repeat(1, 1, self.layer_features_3)
        p3 = m3 * p3
        p3 = torch.flatten(self.avgpool(p3.transpose(1, 2)), 1)
        # p3 = torch.sum(p3, dim=1)
        p2 = self.norm_pyramid2(d2)
        m2 = F.softmax(torch.sum(p2, axis=2), dim=1).unsqueeze(axis=2).repeat(1, 1, self.layer_features_2)
        p2 = m2 * p2
        p2 = torch.flatten(self.avgpool(p2.transpose(1, 2)), 1)
        # p2 = torch.sum(p2, dim=1)
        p1 = self.norm_pyramid1(d1)
        m1 = F.softmax(torch.sum(p1, axis=2), dim=1).unsqueeze(axis=2).repeat(1, 1, self.layer_features_1)
        p1 = m1 * p1
        p1 = torch.flatten(self.avgpool(p1.transpose(1, 2)), 1)
        # p1 = torch.sum(p1, dim=1)

        p2 = p2 + self.uplayer1(p1)
...
class SwinTransformer_Pyramid_M7(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
...
class SwinTransformer_GwL_Pyramid(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, att_size=4, method=None, pretrained_path=None, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
        ...
        self.layer_features_4 = 8 * embed_dim
        self.pyramid_features = 2 * embed_dim
        self.num_features = self.num_features + att_size * self.layer_features_4
        # split image into non-overlapping patches
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        ...
        self.reduction = nn.Linear(self.layer_features_4, self.att_size, bias=False)
        self.apply(self._init_weights)
    ...
    def forward_features(self, x):
        ...
        # p = torch.cat([p4, p3, p2, p1], dim=1)
        # print("p shape: ", p.shape)
        x_global = self.norm(c4)  # B L C
        x_global = self.avgpool(x_global.transpose(1, 2))  # B C 1
        x_global = torch.flatten(x_global, 1)

        # Part Module
        B, L, C = c4.shape
        att_map = self.reduction(c4)
        att_map = torch.sigmoid(att_map)
        att_map_rep = att_map.transpose(1, 2).unsqueeze(axis=3).repeat(1, 1, 1, C)
        x_rep = c3.unsqueeze(axis=1).repeat(1, self.att_size, 1, 1)
        # print("att_map_rep.shape: ", att_map_rep.shape)
        # print("x_rep.shape: ", x_rep.shape)
        c4_local = (att_map_rep * x_rep).view(B * self.att_size, L, C)
        c4_local, _ = self.layers[3](c4_local)
        
        x_local = self.norm(c4_local)
        x_local = self.avgpool(x_local.transpose(1, 2))
        x_local = torch.flatten(x_local, 1)
        x_local = x_local.view(B, self.att_size, C)
        x_all = torch.cat([x_local.view(B, -1), x_global], dim=1)

        c4_local = c4_local.view(B, self.att_size, L, C)
        c4_local_mean = c4_local.mean(axis=3)
        sp_v = F.softmax(c4_local_mean, dim=2)
        ch_v = F.softmax(x_local, dim=2)

        if self.method == 'concat':
            p = torch.cat([x_all, p4], dim=1)
        else:
            x_global = x_global + p4
            p = torch.cat([x_local.view(B, -1), x_local], dim=1)
        return p, sp_v, ch_v
