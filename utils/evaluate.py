...
def mean_average_precision_cont(query_feat,
                           database_feat,
                           query_labels,
                           database_labels,
                           topk=None,
    ...
    Args:
        query_feat (torch.Tensor): Query data feat.
        database_feat (torch.Tensor): Database data feat.
        query_labels (torch.Tensor): Query data targets, one-hot
        database_labels (torch.Tensor): Database data targets, one-host
        device (torch.device): Using CPU or GPU.
        topk (int): Calculate top k data map.
    ...
    query_norm = query_feat.norm(p=2, dim=1)
    database_norm = database_feat.norm(p=2, dim=1)
